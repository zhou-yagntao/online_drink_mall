...
public class CollaborativeFilteringRecommendAlgorith {
     ...
     * @Return: java.util.concurrent.ConcurrentHashMap<java.lang.Long,java.util.concurrent.ConcurrentHashMap<java.lang.Long,java.lang.Long>>
     */
     public ConcurrentHashMap<Long,ConcurrentHashMap<Long,Long>> assembleBehaviors(List<UserActive> userActiveList){
         ...
         for (UserActive userActive:userActiveList){
              ...
              //获得商品id
              Long prodId = userActive.getProductId();
              //获得该编号商品的点击量
              Long prodHits  = userActive.getHits();
     ...
     * @Return: java.util.List<com.zyt.entity.UserSimilarity>
     */
     public List<UserSimilarity> calcSimilarityBetweenUsers(ConcurrentHashMap<Long,ConcurrentHashMap<Long,Long>> activeMap){
         //用户之间的相似度集合
         ...
         for (int i = 0; i < userIdList.size() - 1; i++) {
             for (int j = i+1; j < userIdList.size(); j++) {
                 ...
                 while(iterator_1.hasNext() && iterator_2.hasNext()){
                     ...
                     Long hits_2 = userClickProdHits.get(iterator_2Id);
                     //计算分子
                 ...
                 userSimilarity.setUserRefId(userIdList.get(i));
